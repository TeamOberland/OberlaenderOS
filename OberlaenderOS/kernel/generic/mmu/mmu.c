...
void mmu_switch_to_process(process_t* process)
{
    ...
    mmu_set_master_table(process->masterTable);
}
...
void mmu_create_address_mapping(mmu_table_pointer_t masterTable, uint32_t virtualAddress, uint32_t physicalAddress, uint8_t domain)
{
    ...
    if (l2Table != NULL)
    {
        ...
        l2EntryValue = MMU_L2_TABLE_ENTRY_TO_PAGE(physicalAddress) | 0x00000002;
        access = __mmu_get_domain_access(domain);

        l2EntryValue |= (access << 4);
        l2EntryValue |= (access << 6);
        l2EntryValue |= (access << 8);
        l2EntryValue |= (access << 10);
...
mmu_table_pointer_t mmu_get_or_create_l2_table(mmu_table_pointer_t masterTable, uint32_t masterTableEntry, uint8_t domain)
{
    ...
    if (masterTableEntry < MMU_MASTER_TABLE_PAGE_SIZE)
    {
        ...
        else
        {
            ...
            masterTableEntryValue = ((uint32_t) l2Table) | MMU_CROSS_PAGE_TABLE_INITIAL_VALUE | (domain << 5);
    ...
    if (writeAccess)
    {
        switch (status)
        {
            ...
            case MMU_TRANSLATION_FAULT_PAGE:
                ...
                return  (accessedAddress >= PROCESS_STACK_START && accessedAddress < (PROCESS_STACK_START + PROCESS_STACK_SIZE))
                        || (accessedAddress >= PROCESS_SYSMEM_START && accessedAddress < (PROCESS_SYSMEM_START + PROCESS_SYSMEM_SIZE));
...
void mmu_delete_process_memory(process_t* proc)
{
    ...
    for (masterTableEntry = 0; masterTableEntry < MMU_MASTER_TABLE_PAGE_SIZE; masterTableEntry++)
    {
        ...
        if (l2Table != NULL)
        {
            ...
            for (l2TableEntry = 0; l2TableEntry < MMU_L2_PAGE_COUNT; l2TableEntry++)
            {
